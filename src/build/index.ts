...
async function uploadAssets(): Promise<void> {
  ...
  if (isDebug) {
    ...
    serverLogger.info(`🐛 Debug mode enabled - writing first 100 icons per provider to ${debugDir}`)
  }
  ...
  await Promise.all(
    ICON_PROVIDER_SLUGS.map(async (provider) => {
      ...
      if (isDebug) {
        ...
        serverLogger.info(`🐛 Debug: Wrote ${debugIcons.length} icons for ${provider} to ${debugFile}`)
      }
